# Instalação — Análise Imobiliária Pro v1.6.0
...
2. Subir `imobiliaria.html` (só mudou `?v=1.5.0` → `?v=1.6.0` nos 14 `<script src>`, para furar o cache do Pages).
3. Subir `tests/` (run_imob_v160.js, casos_base.js, baseline_motor_1.2.0.json, run_imob_ui.js). Não há SQL nesta versão — nenhuma tabela, política ou gatilho muda.
4. Conferir: `node tests/run_imob_v160.js` → 92/92 · `npm i --no-save jsdom@24 && node tests/run_imob_ui.js` → 48/48 · na tela, badge v1.6.0.
