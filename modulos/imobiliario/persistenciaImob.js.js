...
(function (raiz) {
  ...
  var imobRepo = {
    ...
    salvarImovel: function (imovel, ctx) {
      ...
      // v1.1.0 — o upsert por id FOI REMOVIDO. Com "excluido_em IS NULL" na politica de
      // SELECT (setup_v7460), o Postgres confere essa politica contra a linha em conflito
      // no ON CONFLICT DO UPDATE: imovel excluido tornaria aquele id ingravavel para sempre
      // (42501, "USING expression") — o mesmo defeito corrigido em atp_analises nos setups
      // v7440/v7450. Aqui nao ha saida de lixeira nem de renome: a chave em conflito e a
      // PRIMARY KEY, referenciada por FK de atp_imob_calculos e atp_imob_creditos.
      // Solucao: nao criar caminho de conflito. Com id -> PATCH; sem id -> INSERT puro.
      if (imovel.id)
        return { metodo: 'PATCH', tabela: TABELAS.imoveis,
                 caminho: TABELAS.imoveis + '?id=eq.' + imovel.id,
                 corpo: corpo, upsert: false, retorna: 'representation',
                 aviso: 'Zero linhas afetadas significa imovel inexistente ou excluido — nao e sucesso.' };
      return { metodo: 'POST', tabela: TABELAS.imoveis,
               caminho: TABELAS.imoveis,
               corpo: [corpo], upsert: false, retorna: 'representation' };
    },
    ...
    exercerOpcao258: function (imovelId, escolha, justificativa, ctx) {
      ...
      // v1.1.0 — a condicao viaja no proprio caminho: se a opcao ja foi exercida, o
      // PostgREST afeta ZERO linhas e nem chega ao gatilho. Reforco do lado do cliente
      // para a trava atp_imob_trava_opcao_raj, que agora tambem congela autor e data.
      return { metodo: 'PATCH', tabela: TABELAS.imoveis,
               caminho: TABELAS.imoveis + '?id=eq.' + imovelId + '&raj_opcao_escolhida=is.null',
               corpo: {
               ...
               aviso: 'A trigger atp_imob_trava_opcao_raj recusa a troca da opcao E a alteracao de autor e data depois de exercida. Zero linhas afetadas = opcao ja exercida.' };
    },
