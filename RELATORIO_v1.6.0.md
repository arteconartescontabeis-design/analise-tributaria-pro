# Análise Imobiliária Pro — v1.6.0 a v1.6.3 · Relatório de implementação
...
## 8. v1.6.3 (19/09/2026) — layout igual ao Análise Tributária Pro · motor e lacre intocados

| Arquivo | Mudança |
|---|---|
| `imobiliaria.html` | Barra lateral no formato do ATP: `brand` "Análise Imobiliária Pro / ARTECON" (mesmo markup do `index.html`), itens dentro de `.nav`, "Sobre o módulo" → "🕘 Versões", link "🏛️ Análise Tributária ↗"; o logotipo sai da barra (segue nos relatórios e no resultado). |
| `imob_layout.js` | As 15 telas do módulo entram na barra com o desenho exato dos `.nav-item` do ATP (14px, 11px 14px, sem recuo, mesmo hover/ativo); separadores Preparar/Calcular/Analisar/Registros em caixa alta discreta; item "Início" duplicado oculto; inserção corrigida para dentro de `.nav`; badge de versão preenchido no título. |
| `imob_pagina.js` | Título da página no padrão do dashboard do ATP: `h1` sem emoji + crumb + `badge ok` com "Análise Imobiliária Pro vX" à direita. |

Suítes: `run_imob_v160.js` 106/106 · `run_imob_ui.js` **53/53** (agora carrega também graficos, marca e layout) · varredura 0 erros.
